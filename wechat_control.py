...
def init():
    ...
    GPIO.setup(CamHorPin, GPIO.OUT)
    GPIO.setup(CamVerPin, GPIO.OUT)
    ...
    pwm_servo.start(0)
    cam_hor_servo = GPIO.PWM(CamHorPin, 50)
    cam_hor_servo.start(0)
    cam_ver_servo = GPIO.PWM(CamVerPin, 50)
    cam_ver_servo.start(0)
    ...
    while True:
        ...
        if size == 0:
            ...
            while size != 0:
                ...
                if StartBit == 1 and serialdatabit == '#'.encode():
                    NewLineReceived = 1
                    InputString = InputStringcache.decode()
                    InputStringcache = ''.encode()
                    StartBit = 0
                    size = 0
                    print (InputString)
